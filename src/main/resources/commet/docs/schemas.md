...
- `paymentMethod` (`PaymentMethod (nullable)`, required) — Method used for this charge: card, oxxo, or mercado_pago. Null when unknown; later changes to the saved method do not alter this transaction.
- `subPaymentMethod` (`SubPaymentMethod (nullable)`, required) — The source of funds for this charge, when reported by the provider. Null when unavailable or unknown.
